...
const orderSchema = new mongoose.Schema({
  ...
  logisticsInfo: {
    ...
    // Rate Limit 保護：記錄上次查詢時間，避免 2 小時內重複查詢
    lastStatusCheck: Date,
    lastStatusCode: String, // 上次查詢的貨態代碼 (301=送達, 201=配送中...)
  },
...
// pre("save") 要放在 mongoose.model 之前註冊
...
orderSchema.pre("save", function (next) {
  ...
  if (this.isModified("items") || this.isNew) {
    if (Array.isArray(this.items)) {
      ...
      const codFee = this.paymentMethod === "COD" ? COD_FEE : 0;

      this.amount = this.subtotal + shipping + codFee;
    }
  }
  next();
});
